...
try:
    from gogo import (
        download_all_files, clean_and_index_cache, build_master_from_index, run_create_master,
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("GoGo Dendro-Dating Tool v8.0")
    ...
    def _create_date_tab(self):
        ...
        def toggle():
            ...
            self.date_treble_label.grid_forget(); self.date_treble_entry.grid_forget(); self.date_treble_browse.grid_forget()
            if is_two_piece:
        ...
        self.date_treble_browse = ttk.Button(frame, text="Browse...", command=lambda: self._browse_file(self.date_treble_entry))
    ...
    def _create_detective_tab(self):
        ...
        def toggle():
            ...
            self.detective_treble_label.grid_forget(); self.detective_treble_entry.grid_forget(); self.detective_treble_browse.grid_forget()
            if is_two_piece:
        ...
        self.detective_treble_browse = ttk.Button(frame, text="Browse...", command=lambda: self._browse_file(self.detective_treble_entry))
        ...
        self.detective_min_overlap_spinbox = ttk.Spinbox(options_frame, from_=30, to=200, increment=10, width=5); self.detective_min_overlap_spinbox.set(80)
    ...
    def _create_index_build_tab(self):
        ...
        index_frame = ttk.LabelFrame(tab, text="Step 1: Download All NOAA Files (Run once)"); index_frame.pack(padx=20, pady=10, fill="x")
        self.index_button = ttk.Button(index_frame, text="Download / Update Files", command=self._run_download); self.index_button.pack(pady=5)
        clean_frame = ttk.LabelFrame(tab, text="Step 2: Clean Cache and Create Index (Run after download)"); clean_frame.pack(padx=20, pady=10, fill="x")
        self.clean_button = ttk.Button(clean_frame, text="Clean and Index Downloaded Files", command=self._run_clean); self.clean_button.pack(pady=5)
        build_frame = ttk.LabelFrame(tab, text="Step 3: Build Master Chronologies from Index"); build_frame.pack(padx=20, pady=10, fill="x")
    ...
    def _browse_for_sample_file(self):
        ...
        if filename: self.date_sample_entry.delete(0, tk.END); self.date_sample_entry.insert(0, filename); self.detective_sample_entry.delete(0, tk.END); self.detective_sample_entry.insert(0, filename)
    ...
    def _run_date(self):
        ...
        if analysis_type == "single":
            ...
            self._run_in_thread(run_date_analysis, (sample, master, min_overlap), run_button, is_analysis=True)
        else:
            ...
            final_args = ["placeholder", master, min_overlap]
            self._run_in_thread(run_two_piece_mean_analysis, (bass, treble, run_date_analysis, final_args), run_button, is_analysis=True)
    ...
    def _run_detective(self):
        ...
        if analysis_type == "single":
            ...
            if not sample: messagebox.showerror("Error", "Please select a sample file."); return
            self._run_in_thread(run_detective_analysis, (sample, target, top_n, min_overlap), run_button, is_analysis=True)
        else:
            bass = self.detective_sample_entry.get(); treble = self.detective_treble_entry.get()
            if not bass or not treble: messagebox.showerror("Error", "Please select both bass and treble files."); return
            final_args = ["placeholder", target, top_n, min_overlap]
            self._run_in_thread(run_two_piece_mean_analysis, (bass, treble, run_detective_analysis, final_args), run_button, is_analysis=True)
    ...
    def _run_download(self):
        self._run_in_thread(download_all_files, (), self.index_button)
        
    def _run_clean(self):
        self._run_in_thread(clean_and_index_cache, (), self.clean_button)
        
    def _run_build(self):
        target = self.build_target_combo.get()
        if target in ['alpine', 'all']: self._run_in_thread(build_master_from_index, ("Alpine Instrument Wood", ['PICEA', 'ABIES'], ['aust', 'fran', 'germ', 'ital', 'swit', 'slov'], 150, 1750), self.build_button)
        if target in ['baltic', 'all']: self._run_in_thread(build_master_from_index, ("Baltic Northern Timber", ['PINUS', 'PICEA'], ['finl', 'germ', 'lith', 'norw', 'pola', 'swed'], 150, 1750), self.build_button)
